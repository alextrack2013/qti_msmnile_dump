...
function configure_zram_parameters() {
	...
	if [ -f /sys/block/zram0/disksize ]; then
		...
		if [ $MemTotal -le 524288 ]; then
			...
			if [ "$target" == "kona" ] || [ "$target" == "lahaina" ]; then
				#config 5GB zramsize with ramsize over 8GB
				swap_size_mb=5120
				zram_increase_limit=2048
			elif [ "$target" == "msmnile" ]; then
				#config 5GB zramsize with ramsize over 8GB
				swap_size_mb=5120
				zram_increase_limit=2048
			else
...
function configure_swappiness()
{
	...
	local oplus_path=oplus_healthinfo
	...
	if [ "$fn_enable" == "true" -a "$nandswap_size" != "0" -a $hybridswap_has_insmod -eq 1 ]; then
		...
		if [ "$target" == "kona" ] || [ "$target" == "lahaina" ] && [[ $MemTotal -gt 6291456 ]]; then
			echo "direct_swappiness=0" > /proc/${oplus_path}/swappiness_para
			echo "vm_swappiness=200" > /proc/${oplus_path}/swappiness_para
		fi
	else
		echo "direct_swappiness=60" > /proc/${oplus_path}/swappiness_para
	fi
